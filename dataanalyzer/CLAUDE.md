...
datagen이 생성한 JSONL 파일을 분석해 tool 분포, 토큰 수, 파라미터 커버리지 등 7가지 분석 항목의 차트와 텍스트 리포트를 생성한다. 학습 전 데이터 품질을 시각적으로 확인하는 용도다.
...
- `datagen/tool_specs.py`의 `tools` 명세에서 optional 파라미터 목록을 가져온다 — 함수 스키마 변경 시 분석 결과도 자동 반영
