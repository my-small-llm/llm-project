...
`2_root_cause`의 마지막 문서다.

앞선 문서들이 수정 전 실패를 데이터셋 규칙과 함수 스키마 수준에서 추적했다면, 이 문서는 그보다 한 단계 더 나아가 최신 tool-calling / PEFT 연구 관점에서 `train_data`를 다시 읽는다. 즉 이미 일부 데이터를 고친 뒤에도 만족스럽지 않았던 지점을, "앞으로 어떤 구조의 데이터셋이 더 좋은 학습 신호를 만들 것인가"라는 질문으로 확장해 분석한 연구 문서다.
