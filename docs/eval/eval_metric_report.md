...
| 60~75%    | 90%+      | 도메인 갭 존재               | 실제 데이터 혼합 비율 ↑ |
...
| ----------------------------------------------------------- | ------ | ------- | ----------------------- |
| `search_restaurants(keyword="치킨", sort_by="rating")`      | O      | O       | 완벽                    |
| `search_restaurants(keyword="chicken", sort_by="distance")` | X      | X       | 구조는 맞지만 값이 다름 |
| `get_menu_items(restaurant_id="xxx")`                       | X      | X       | 아예 함수가 잘못됨      |
...
| 모델 출력                            | IrrelAcc | 설명        |
| ------------------------------------ | -------- | ----------- |
| 호출 안 함 (거부 + 안내)             | O        | 올바른 거부 |
| `search_restaurants(keyword="보험")` | X        | 과잉 호출   |
...
| 메트릭                    | 질문                         | 예시                                                         |
| ------------------------- | ---------------------------- | ------------------------------------------------------------ |
| **Tool Choice**           | 함수 선택이 맞았나?          | `search_restaurants` 불러야 하는데 `get_menu_items` 부름 → X |
| **Param Name Recall**     | 필수 인자를 빠짐없이 넣었나? | `keyword` 필수인데 안 넣음 → X                               |
| **Param Name Precision**  | 쓸데없는 인자를 넣지 않았나? | `include_soldout`을 멋대로 추가 → 감점                       |
| **Param Value Precision** | 값이 맞나?                   | `keyword="chicken"` (치킨이어야 함) → X                      |
| **Schema Validation**     | 타입이 스키마에 맞나?        | `quantity`가 정수여야 하는데 문자열로 넣음 → X               |
| **Exact Match**           | 전부 완벽히 동일한가?        | 위의 모든 항목이 다 O일 때만 O                               |
...
```
search_restaurants == search_restaurants → O (1/1 = 100%)
```
...
예측에 있는가: query O / category X → Recall = 1/2 = 50%
```
...
```
예측 인자: {query, limit}
정답에 있는가: query O / limit X → Precision = 1/2 = 50%
```
...
예측 인자: {query, limit} → limit이 없음! → X Invalid (0%)
```

**⑥ Exact Match (ASTAcc)** — 전체가 완전 동일한지 (All-or-Nothing)

```
정답 ≠ 예측 (category↔limit 불일치) → X (0%)
```

**⑦ Relevance Detection F1** — 아래 상황에서 계산

```
Case A: 정답="주문 완료!" 예측="주문 완료!"  → O TN (올바른 비호출)
Case B: 정답="주문 완료!" 예측=tool_call(...)  → X FP (불필요한 호출)
Case C: 정답=tool_call(...) 예측="잠시만요"  → X FN (호출 누락)
F1 = 2·P·R / (P+R)
```
